import { useState } from "react";
import { cn } from "@/lib/utils";

const skills = [
  // Frontend
  { name: "HTML/CSS", level: 70, category: "frontend" },
  { name: "JavaScript", level: 70, category: "frontend" },
  { name: "Tailwind CSS", level: 50, category: "frontend" },
  { name: "Vanilla.js", level: 50, category: "frontend" },
  { name: "Next.js", level: 50, category: "frontend" },
  { name: "React", level: 20, category: "frontend" },
  { name: "TypeScript", level: 20, category: "frontend" },

  // Backend
  { name: "C#", level: 70, category: "backend" },
  { name: "Java", level: 70, category: "backend" },
    { name: "Node.js", level: 70, category: "backend" },
  { name: "Express.js", level: 70, category: "backend" },
  { name: "Kotlin", level: 50, category: "backend" },
  { name: "C++", level: 50, category: "backend" },

  // Frameworks
  { name: ".NET", level: 70, category: "frameworks" },
  { name: "Express.js", level: 70, category: "frameworks" },
  { name: "WPF", level: 50, category: "frameworks" },
  { name: "WCF", level: 50, category: "frameworks" },
  { name: "Tailwind CSS", level: 50, category: "frameworks" },
  
  // Database
  { name: "MySQL", level: 70, category: "database" },
  { name: "SQL Server", level: 70, category: "database" },
  { name: "MongoDB", level: 20, category: "database" },
  { name: "Google Firestore", level: 20, category: "database" },

  // Tools
  { name: "Git/GitHub", level: 100, category: "tools" },
  { name: "Figma", level: 100, category: "tools" },
  { name: "Swagger", level: 100, category: "tools" },
  { name: "Trello", level: 100, category: "tools" },
  { name: "Slack", level: 100, category: "tools" },
  { name: "Postman", level: 100, category: "tools" },
  { name: "Docker", level: 50, category: "tools" },
];

const categories = ["all", "frontend", "backend", "frameworks", "database", "tools"];

export const SkillsSection = () => {
  const [activeCategory, setActiveCategory] = useState("all");

  const filteredSkills = skills.filter(
    (skill) => activeCategory === "all" || skill.category === activeCategory
  );
  return (
    <section id="skills" className="py-24 px-4 relative bg-secondary/30">
      <div className="container mx-auto max-w-5xl">
        <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center">
          My <span className="text-primary"> Skills</span>
        </h2>

        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {categories.map((category, key) => (
            <button
              key={key}
              onClick={() => setActiveCategory(category)}
              className={cn(
                "px-5 py-2 rounded-full transition-colors duration-300 capitalize",
                activeCategory === category
                  ? "bg-primary text-primary-foreground"
                  : "bg-secondary/70 text-forefround hover:bd-secondary"
              )}
            >
              {category}
            </button>
          ))}
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSkills.map((skill, key) => (
            <div
              key={key}
              className="bg-card p-6 rounded-lg shadow-xs card-hover"
            >
              <div className="text-left mb-4">
                <h3 className="font-semibold text-lg"> {skill.name}</h3>
              </div>
              <div className="w-full bg-secondary/50 h-2 rounded-full overflow-hidden">
                <div
                  className="bg-primary h-2 rounded-full origin-left animate-[grow_1.5s_ease-out]"
                  style={{ width: skill.level + "%" }}
                />
              </div>

              <div className="text-right mt-1">
                <span className="text-sm text-muted-foreground">
                  {skill.level}%
                </span>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};